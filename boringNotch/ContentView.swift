                ...
                mainLayout
                    ...
                    .onReceive(NotificationCenter.default.publisher(for: .sharingDidFinish)) { _ in
                        if vm.notchState == .open && !isHovering
                            ...
                            && !vm.isProcessDetailPopoverActive {
                            hoverTask?.cancel()
                            hoverTask = Task {
                                ...
                                await MainActor.run {
                                    if self.vm.notchState == .open && !self.isHovering
                                        ...
                                        && !self.vm.isProcessDetailPopoverActive
                    ...
                    .onChange(of: vm.isBatteryPopoverActive) {
                        if !vm.isBatteryPopoverActive && !vm.isProcessDetailPopoverActive
                            ...
                            hoverTask = Task {
                                ...
                                await MainActor.run {
                                    if !self.vm.isBatteryPopoverActive
                                        && !self.vm.isProcessDetailPopoverActive
                                        && !self.isHovering && self.vm.notchState == .open
                                        && !SharingStateManager.shared.preventNotchClose {
                                        self.vm.close()
                                    }
                                }
            ...
            hoverTask = Task {
                ...
                await MainActor.run {
                    ...
                    if self.vm.notchState == .open
                        ...
                        && !self.vm.isProcessDetailPopoverActive
                        && !SharingStateManager.shared.preventNotchClose
                        && !IslandWindowManager.shared.isHoveringIsland
                        && !IslandWindowManager.shared.isMouseOverIsland {
                        self.vm.close()
                    }
                }
            }
